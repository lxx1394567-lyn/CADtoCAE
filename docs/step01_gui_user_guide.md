...
人工校核时优先修改 `原始材料表`。`建模构件表` 中的 `构件名称`、`abaqus_part_name`、`规格`、`长度_mm`、`长度_m`、`数量`、`材料牌号`、`建模方式` 和 `单元类型` 会随 `原始材料表` 同行内容自动更新。

如果自动规则不符合实际构件，可直接在 `建模构件表` 对应单元格手工覆盖；Step02 会优先使用手工覆盖值。`截面类型`、`截面参数`、`厚度_mm`、`厚度_m` 主要用于查看初始解析结果，Step02 会重新从 `规格` 列解析截面。
